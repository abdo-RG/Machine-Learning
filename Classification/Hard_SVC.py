...
#------------------------------------------constraints----------------------------------------------#
cons = ()
for i,j in zip(train_set,y_train):
    cons = cons + ({'type': 'ineq', 'fun': lambda w,i=i,j=j:  i[0]*j*w[0] +i[1]*j*w[1]+j*w[2]-1},)

#---------------------- fonction à minimiser ---------------------------------------------------------#
fun = lambda w: 0.5*(w[0]**2+w[1]**2)
#--------------------- maximisation de la marge <===> minization using SLSQP méthode ------------------------------------------------#
res = minimize(fun, np.array([1, 1,1]), method='SLSQP',jac='2-point',constraints=cons)
theta=res.x
